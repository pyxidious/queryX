...
def backfill_virtual_assets(settings: Settings) -> None:
    """Promote current historical external-source scans without blocking startup."""
    try:
        ...
        catalog.backfill_mongodb_assets(sources)
    except Exception:
        logger.exception("Catalog virtual asset backfill failed")


def backfill_mysql_assets(settings: Settings) -> None:
    """Backward-compatible entry point for startup and focused tests."""
