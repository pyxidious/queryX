...
class IngestionStorage:
    ...
    def sync_mysql_assets(
        self, source_id: str, promoted: list[dict[str, Any]]
    ) -> None:
        """Synchronize virtual MySQL table assets without creating storage bindings."""
        now = _now().isoformat()
        current_ids = {item["asset_id"] for item in promoted}
        with self._connect() as connection:
            connection.execute("BEGIN IMMEDIATE")
            mysql_assets = connection.execute(
                "SELECT id FROM data_assets WHERE asset_kind = 'mysql_table'"
            ).fetchall()
            for asset_row in mysql_assets:
            ...
            for item in promoted:
                description = f"Virtual MySQL table {source_id}.{item['name']}"
                connection.execute(
